...
public class ElevensBoard11 extends Board11 {
	...
	private List<Integer> findJQK(List<Integer> selectedCards) {
		...
		for (Integer kObj : selectedCards) {
			int k = kObj.intValue();
			if (cardAt(k).rank().equals("jack") && jackIndex == null) {
				jackIndex = k;
			} else if (cardAt(k).rank().equals("queen") && queenIndex == null) {
				queenIndex = k;
			} else if (cardAt(k).rank().equals("king") && kingIndex == null) {
				kingIndex = k;
			}
		}
		if (jackIndex != null && queenIndex != null && kingIndex!= null)
		{
			indexes.add(jackIndex);
			indexes.add(queenIndex);
			indexes.add(kingIndex);
